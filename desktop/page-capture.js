...
(async () => {
  ...
  function boxStyle(style, fallbackFill = null) {
    ...
    const borderColor = rgbaToHex(style.borderTopColor);
    if (borderWidth > 0 && borderColor && style.borderTopStyle !== "none") {
      result.stroke = borderColor;
      result.strokeWidth = round(borderWidth);
    }
  ...
  await Promise.all(
    Array.from(rootElement.querySelectorAll("img"))
      ...
      .map(async (element) => {
        ...
        if (externalSvg) externalSvgByElement.set(element, externalSvg);
      })
  );
