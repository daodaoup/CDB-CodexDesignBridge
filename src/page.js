import { computeHash, prepareSvgAsset } from "./svg.js";
...
export function preparePageManifest({ json, sourcePath }) {
  ...
  const normalizeNode = (node, path) => {
    ...
    if (node.type === "frame") {
      ...
      normalized.objectFit = optionalEnum(node.objectFit, ["fill", "contain", "cover", "none", "scale-down"], "cover", `${path}.objectFit`);
    } else {
...
function normalizeImage(image, path) {
  ...
  if (typeof image.base64 !== "string" || image.base64.length === 0 || image.base64.length > 1536 * 1024 || !/^(?:[A-Za-z0-9+/]{4})*(?:[A-Za-z0-9+/]{2}==|[A-Za-z0-9+/]{3}=)?$/.test(image.base64)) {
...
function normalizeLayoutItem(layoutItem, path) {
  if (!layoutItem || typeof layoutItem !== "object" || Array.isArray(layoutItem)) {
    throw new PageValidationError(`${path} must be an object.`);
  }
  ...
  return {
    align: optionalEnum(layoutItem.align, ["auto", "start", "center", "end", "stretch"], "auto", `${path}.align`),
    grow: boundedNumber(layoutItem.grow, 0, 0, 100, `${path}.grow`),
    shrink: boundedNumber(layoutItem.shrink, 1, 0, 100, `${path}.shrink`),
    basis: optionalString(layoutItem.basis, "auto", `${path}.basis`, 80),
    order,
    positioning: optionalEnum(layoutItem.positioning, ["auto", "absolute"], "auto", `${path}.positioning`),
    horizontalSizing: optionalEnum(layoutItem.horizontalSizing, ["fixed", "hug", "fill"], "fixed", `${path}.horizontalSizing`),
    verticalSizing: optionalEnum(layoutItem.verticalSizing, ["fixed", "hug", "fill"], "fixed", `${path}.verticalSizing`),
    gridRow: optionalString(layoutItem.gridRow, "auto", `${path}.gridRow`, 80),
    gridColumn: optionalString(layoutItem.gridColumn, "auto", `${path}.gridColumn`, 80),
  };
}
